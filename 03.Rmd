...
- Understand what *is* `{crosstalk}` and how to deploy it to `{shiny}`

`{htmlwidgets}` is an a succsseor to `{rCharts}`. 
...
However, is `{ggplot}` our only graphic utiltiy/package/service? Certainly NO! Let's discuss [plotly](https://cran.r-project.org/web/packages/plotly/index.html).
